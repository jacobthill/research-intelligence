...
with open('output/match_dois_to_sites.csv', mode='w') as out:
    out = csv.writer(out, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    # update
    out.writerow(["title", "authors", "id", "doi", "publisher", "journal", "volume", "issue", "pages", "pub_year", "concepts", "abstract", "acknowledgements", "funders", "research_org_cities", "research_org_names", "research_org_state_names", "research_orgs", "provenance", "pub_harvested_date"])

    for c in range(chunks):
        query = dsl.query("""search publications where doi in {} return publications [title + researchers + id + doi + publisher + journal + volume + issue + pages + year + concepts + abstract + acknowledgements + funders + research_org_cities + research_org_names + research_org_state_names + research_orgs] limit 1000""".format(json.dumps(list(input.doi)[start:end])))
        ...
        for pub in query['publications']:
            ...
            out.writerow([pub['title'], '; '.join(researcher_ids), pub['id'], pub.get('doi'), pub.get('publisher'), pub.get('journal'), pub.get('volume'), pub.get('issue'), pub.get('pages'), pub.get('year'), pub.get('concepts'), pub.get('abstract'), pub.get('acknowledgements'), pub.get('funders'), pub.get('research_org_cities'), pub.get('research_org_names'), pub.get('research_org_state_names'), pub.get('research_orgs'), 'dimensions', datetime.now().strftime("%d/%m/%Y %H:%M:%S")])

        start+=chunk_size
        end+=chunk_size

    query = dsl.query("""search publications where doi in {} return publications [title + researchers + id + doi + publisher + journal + volume + issue + pages + year + concepts + abstract + acknowledgements + funders + research_org_cities + research_org_names + research_org_state_names + research_orgs] limit 1000""".format(json.dumps(list(input.doi)[start:])))
